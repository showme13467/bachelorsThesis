...
import {Layer, Circle, Stage, Text} from 'react-konva'
...
class DevicesUpdateCoordinates extends Component {
    constructor(props) {
        super(props)
...
const a = []
const b = []
for(var i = 0 ; i < room.pixel.length; i ++){
if((i%2)=== 0 ){
a.push(room.pixel[i])
}
else {
...
const x = Math.min.apply(Math, a)
const y = Math.min.apply(Math, b)
...
return y
    ...
    render(){
      const {devices, room, device} = this.state
      ...
      return (
           <Wrapper>
                  ...
                  <Divimg>
                <ImageMapper onMouseenter={area => this.enterArea(area)} onMouseLeave={area => this.leaveArea(area)}  src={URL} map={MAP} width={1211} height={800}  alt="Floorplan"/>

                </Divimg>                

              { showDevices && ( <Stage opacity={1} offsetX ={-367} offsetY ={-121} width={1211} height={800}>
        <Layer>
            ...
            onDragEnd={e => {
              ...
              this.saveNewPixel();
                    ...
                    <Text text={room.name} x={this.getMiddlePoint("x")+ 10} y={this.getMiddlePoint("y")+ 10} fontSize={20}/>
...
export default DevicesUpdateCoordinates
